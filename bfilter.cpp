...
///ranges: the set of multidimensional ranges, the filter is writen to fdata with the beginning byte of startbyte
int Bfilter::construct_Bloomfilter(int chunkid, vector<uint64_t> ranges){
    ...
    for(i = 0; i < ranges.size(); i++){
        for(j = 0; j < k; j++){
            ...
            p = MurmurHash3_x86_32(data, strlen(c), j, ranges[i]);
            beginbyte = p / BYTE % bytenum;
            beginbit = p % BYTE;
...
///------------------------------------------------------------------------------------------------------------------------------------------
int Bfilter::search_Bloomfilter(string sbitmap, uint64_t srange, int fbyte){
    int i, j;
    uint32_t p, u;
    const char* c = to_string(srange).c_str();
    const uint8_t* data = (const uint8_t*)c;
    for(j = 0; j < k; j++){
        p = MurmurHash3_x86_32(data, strlen(c), j, srange);
        beginbyte = p / BYTE % fbyte;
        beginbit = p % BYTE;
        u = sbitmap[beginbyte];
...
///------------------------------------------------------------------------------------------------------------------------------------------
uint32_t Bfilter::MurmurHash3_x86_32(const uint8_t* key, int len, uint32_t seed, uint64_t r) {
